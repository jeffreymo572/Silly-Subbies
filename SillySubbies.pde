...
void move() {
    ...
    if (inputs[4]) {
        bullets.add(new Bullet(submarine.x + 50, submarine.y + 30));
        inputs[4] = !inputs[4];
    }
}
...
void playGame() {
  ...
  midground_Layer_2(); //Contains Octopi
  midground_Layer_1(); //Contains Bullets
  
  foreground_Layer(); //Contains submarine and GUI
}
